...
def update_soul_txt(public_url):
    with open("cyber3.txt", "w") as file:
...
def update_soul_txt(public_url):
    with open("cyber3.txt", "w") as file:
        file.write(public_url)
    print(f"New ngrok link saved in cyber3.txt")
...
def update_vps_soul_txt(public_url):
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        with sftp.open("cyber3.txt", "w") as file:
            file.write(public_url)
        sftp.close()
        ssh.close()
        print("Updated cyber3.txt on VPS successfully.")
    except Exception as e:
        print(f"Failed to update cyber3.txt on VPS: {str(e)}")
