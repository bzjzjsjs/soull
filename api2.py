...
def update_soul_txt(public_url):
    with open("aryan2.txt", "w") as file:
        file.write(public_url)
    print(f"New ngrok link saved in aryan2.txt")
...
def update_vps_soul_txt(public_url):
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        with sftp.open("aryan2.txt", "w") as file:
            file.write(public_url)
        sftp.close()
        ssh.close()
        print("Updated aryan2.txt on VPS successfully.")
    except Exception as e:
        print(f"Failed to update aryan2.txt on VPS: {str(e)}")
