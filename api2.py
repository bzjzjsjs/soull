...
def configure_ngrok():
    ngrok_token = "2sAI6K0Mb559vkWyKmFAFJLKWhA_5QqkHoxEJdCqDE8vYpuZC"
    while True:
        try:
...
def update_soul_txt(public_url):
    with open("cyber2.txt", "w") as file:
...
def update_soul_txt(public_url):
    with open("cyber2.txt", "w") as file:
        file.write(public_url)
    print(f"New ngrok link saved in cyber2.txt")
...
def update_vps_soul_txt(public_url):
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        with sftp.open("cyber2.txt", "w") as file:
            file.write(public_url)
        sftp.close()
        ssh.close()
        print("Updated cyber2.txt on VPS successfully.")
    except Exception as e:
        print(f"Failed to update cyber2.txt on VPS: {str(e)}")
